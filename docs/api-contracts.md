...
- `accept_couple_invite(invite_token)`
  ...
  - A concurrent second-accept that loses the active-role unique-index race also raises `COUPLE_FULL` (the membership insert re-raises it), so the invite loser never surfaces a raw unique-violation
