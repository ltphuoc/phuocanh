...
| --------------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sendMagicLinkAction`             | `email`, `locale?`, `next?`                                                                        | `ActionState`                                | Requests Supabase magic link; derives `emailRedirectTo` from server-trusted `getSiteUrl()` (client-sent origin is ignored); preserves a valid internal `next` path through the auth callback; no revalidation                                                                                         |
...
| `createInviteAction`              | `email`                                                                                            | `ActionStateWithData<{ inviteUrl: string }>` | Creates a 14-day invite URL bound to the partner email (stored `lower(btrim(email))` as `invited_email`); rejects an invalid email with `auth.invite.invalidEmail` before any write                                                                                                                   |
| `acceptInviteAction`              | `token`                                                                                            | `ActionState`                                | Must call `accept_couple_invite(...)`; no direct membership writes; maps `INVITE_ALREADY_MEMBER` to an info notice (token not consumed)                                                                                                                                                               |
| `createMemoryAction`              | `memoryId`, `happenedAt`, location fields, `note?`, media metadata arrays                          | `ActionState`                                | Requires note or at least one media item; max `25MB` per file; supports multiple image/video media rows; whole-submission atomic (rolls back uploaded objects + memory row on failure); revalidates memory-derived routes                                                                             |
| `updateMemoryAction`              | `memoryId`, `happenedAt`, location fields, `note?`, removed media IDs, added media metadata arrays | `ActionState`                                | Calls `update_memory_media(...)`; atomically updates memory data/media, enforces note-or-media invariant, returns removed storage paths and affected album IDs for post-commit cleanup; revalidates memory-derived routes                                                                             |
...
| `updateTripAction`                | `tripId`, `title`, `startDate`, `endDate`, `note?`, location fields                                | `ActionState`                                | Updates trip fields; rejects date ranges that exclude existing stops (`trip.invalidDateRangeWithStops`) or that would orphan album photos whose memory date falls outside the new range (`trip.invalidDateRangeWithAlbumMedia`)                                                                       |
...
| `updateCoupleTimezoneAction`      | `timeZone`                                                                                         | `ActionState`                                | Calls `update_couple_timezone(...)`; revalidates `/settings`, `/home`, `/on-this-day`, `/countdowns`, `/future-notes`, `/trips`, `/albums`, and `/map`. The client gates a _changed_ zone behind an explicit confirmation (the RPC clears not-yet-revealed game rounds)                               |
| `eraseCoupleSpaceAction`          | `confirmation=DELETE`                                                                              | `ActionState`                                | Irreversible. Reads `memory_media.storage_path`, wipes the couple's `memory-media` objects, THEN calls `erase_couple_space()` (storage-before-DB keeps it retryable); returns `settings.couple.erased`. No direct `couples`/`couple_memberships` writes; the user stays authenticated and re-onboards |
| `ensureDailyQuestionRoundAction`  | `locale`                                                                                           | `ActionState`                                | Generates one prompt through the Google Gemini API, calls `ensure_daily_question_round(...)`, and revalidates `/games`, `/games/daily-question`, and `/stats`                                                                                                                                         |
...
- `accept_couple_invite(invite_token)`
  ...
  - Only opens unused invites (`accepted_at is null`)
  - Raises `INVITE_ALREADY_MEMBER` when the caller is already an active member, without consuming the token (creator self-open no longer burns the invite); also `INVITE_NOT_FOUND`, `INVITE_EXPIRED`, `COUPLE_FULL`, `AUTH_REQUIRED`
  - Email binding: when the invite carries an `invited_email`, raises `INVITE_EMAIL_MISMATCH` (leaving the token unconsumed) unless the accepter's `auth.users` email matches it (both `lower(btrim(...))`-normalized; a null accepter email is also rejected for a bound invite). Checked **before** `COUPLE_FULL`, so a wrong-email stranger cannot learn couple occupancy. The accepter email is read from `auth.users`, never from caller input
  - A concurrent second-accept that loses the race also raises `COUPLE_FULL`: the membership insert maps both the active-role unique-violation and the max-two-active-members trigger error to `COUPLE_FULL`, so the loser never surfaces a raw error
- `update_couple_timezone(target_couple_id, target_timezone)`
  ...
  - Reconciles game rounds in the same transaction: deletes not-yet-revealed rounds dated today-or-future under the old zone (per-mode reveal threshold) so a date shift cannot strand an in-flight round; preserves revealed and past rounds
...
- `update_memory_media(p_memory_id, p_note, p_happened_at, p_location_address, p_location_latitude, p_location_longitude, p_location_name, p_location_provider, p_location_provider_id, p_remove_media_ids, p_add_media)`
  - Memory update atomic transaction path only
  - Holds a row lock on the memory to serialize concurrent edits of the same memory
  - Enforces couple membership re-assertion (SECURITY DEFINER bypasses RLS)
  - Updates memory row, removes specified media, inserts new media (couple_id/memory_id forced server-side)
  - Enforces content invariant: memory must have a note OR at least one media item
  - Returns removed storage paths and affected album IDs for post-commit cleanup
- `delete_empty_albums(p_couple_id, p_album_ids)`
  - Atomic empty-album cleanup path only (`SECURITY INVOKER`; the `albums_delete` RLS policy + grants gate it — no new privileged surface)
  - Single statement: deletes the candidate albums that have zero `album_items` at delete time, so a partner's concurrent `add_album_items` can never be lost to a stale emptiness check (PostgREST cannot express this correlated `NOT EXISTS` anti-join)
- `erase_couple_space()`
  - Member-only, irreversible couple-data erasure path (`SECURITY DEFINER`); resolves the caller's couple from their own active membership, else `NOT_A_MEMBER`
  - Takes the `hashtext('bootstrap_first_couple')` advisory lock and a `for update` row lock, then deletes `game_rounds` first (cascading the `*_targets` whose `ON DELETE RESTRICT` refs to memories would otherwise abort) and finally `couples` (cascading the rest of the couple-scoped graph)
  - Storage objects are wiped by `eraseCoupleSpaceAction` before this runs; auth users are untouched
  - Raises `FORBIDDEN` if caller is not a couple member, `MEMORY_NOT_FOUND` if memory is missing, `MEMORY_REQUIRES_CONTENT` if content invariant is violated
...
- `has_any_couple()`
  ...
  - Security-definer boolean RPC; the gate calls it unconditionally (no service-role admin client on the user-triggered auth path)
