...
export const LoginForm = ({ initialNextPath }: LoginFormProps): ReactElement => {
  const { locale } = useI18n();
  const { t: actionsT } = useI18n('actions');
  const { t: commonT } = useI18n('common');
  const { t: formT } = useI18n('forms.login');
  const [state, submitAction, isPending] = useActionState(sendMagicLinkAction, initialActionState);
