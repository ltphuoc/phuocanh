...
const getMembershipForUser = async (userId: string): Promise<MembershipRow | null> => {
...
export const getReadyCoupleContextOrRedirect = async (locale: Locale): Promise<CoupleContext> => {
  const state = await getAuthGateState();
