...
export const generateMetadata = async ({ params }: MapPageProps): Promise<Metadata> =>
  getRouteMetadata(params, 'map');

export default async function MapPage({ params }: MapPageProps): Promise<ReactElement> {
  const locale = await resolveLocaleFromParams(params);
  const context = await getReadyCoupleContextOrRedirect(locale);
  const dehydratedState = dehydrateAppQuery(appQueryKeys.map(), await getMapAppData(context));
