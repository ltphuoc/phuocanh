...
const findLatestMessageId = async (email: string): Promise<string | null> => {
  const searchResult = mailpitSearchResponseSchema.parse(
    await fetchJson(`${MAILPIT_API_BASE_URL}/search?query=${encodeURIComponent(email)}`),
  );
...
export const waitForMagicLinkUrl = async (email: string, timeoutMs = 30_000): Promise<string> => {
  const deadline = Date.now() + timeoutMs;

  while (Date.now() < deadline) {
    const messageId = await findLatestMessageId(email);
    if (messageId) {
      const detail = mailpitMessageDetailSchema.parse(
        await fetchJson(`${MAILPIT_API_BASE_URL}/message/${messageId}`),
      );
...
export const waitForMagicLinkCode = async (email: string, timeoutMs = 30_000): Promise<string> => {
  const deadline = Date.now() + timeoutMs;
