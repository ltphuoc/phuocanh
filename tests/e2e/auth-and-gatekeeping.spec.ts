...
test("E2E-ROUTE-001 invalid detail routes render not found", async ({ page }) => {
  for (const path of [
    "/en/memories/not-a-uuid",
    "/en/trips/not-a-uuid",
    "/en/albums/not-a-uuid",
  ]) {
...
test("E2E-AUTH-003 invite login preserves token through the Mailpit magic link", async ({
  ...
  await expect(guestPage).toHaveURL(new RegExp(`/en/login\\?next=${encodeURIComponent(invitePath)}`));
  ...
  await expect(
    callbackPage,
  ).toHaveURL(new RegExp(`${invitePath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`));
